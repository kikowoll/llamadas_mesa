...
function btn_menu(dato) { // cambio de pagina (llamadas, grupos y admin
    switch (dato) {
        case 'llamadas':
...
// hoar y dia
function horaca(hora) { // obtiene la hora actual
    x = new Date(),
    h = x.getHours(),
    m = x.getMinutes()
    if(m.toString().length < 2) 
        return hora = h + ":0" + m.toString()
    return hora = h + ':' + m
} 

function diaca(fecha) { // obtiene el dia actual
    var x = new Date(),
        d = x.getDate(),
        m = x.getMonth(),
        a = x.getFullYear()
 ...
 ************************  SERVIDOR *******************************************
 ******************************************************************************/
function empezarServidor() { // llena el select con los servidores
    $.ajax({
        url: direcion,
        type: 'POST',
        dataType: 'JSON',
...
function selecionServidor() { // transicion del servidor
...
function finalServidor() { // envia datos servidor y cierra el servicio
    Swal.fire({
 ...
 ************************  CREAR LLAMADA **************************************
 ******************************************************************************/
function crearLlamada() { // abre el formulario y llena los selects
    $.ajax({
        url: direcion,
        type: 'POST',
        dataType: 'JSON',
...
$('.btn-relac').on('click', function() { // animacion de los botones relacion
...
function mandarLlamada() { // guarda la llamada
    $.ajax({
        url: direcion,
        type: 'POST',
        dataType: 'HTML',
...
function mostrarLlamada() { // muestra las llamadas del mismo servidor dia
    $.ajax({
        url: direcion,
        type: 'POST',
        dataType: 'JSON',
...
function mostrarLlamadaTotal() { // muestra rodas las llamads del mismo dia
    $.ajax({
        url: direcion,
        type: 'POST',
        dataType: 'JSON',
...
// funciones extras
function limpiarLlamadas() { // limpia el formuklario de llamadas
...
// animacion llamadas
$(function() {
    $('#tabla-llamadas #conte-llamadas').on('click','tr>.datos', function() { // animacion de las llamadas
        var aa = $(this), 
...
function borrarDatos() { // borrar elementos de grupos
